...
const PLUGIN_PAGES: Record<string, PluginPage> = {
  analytics: {
    navLabel: 'Dashboard',
    path: '/',
    expectedTexts: ['Data Readiness Desk', 'Field Coverage'],
  },
  lakebase: {
    navLabel: 'Review Queue',
    path: '/queue',
    expectedTexts: ['Review Queue'],
  },
  genie: {
    navLabel: 'Genie',
    path: '/genie',
...
test('smoke test - app loads and displays home page', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Data Readiness Desk' })).toBeVisible();
  await expect(page.getByText('Field Coverage')).toBeVisible();
